...
const initialPositions = () => {
  ...
  const cx = ox + pw / 2;
  const wide = pw * 0.38;
  const hGK = oy + ph - 30, hFB = oy + ph - 75, hHB = oy + ph - 175;
  const hMid = oy + ph / 2 + 10, hHF = oy + 175, hFF = oy + 75;
  const home = [
...
export default function GAAWhiteboard() {
  ...
  const [players, setPlayers] = useState(() => { const p = initialPositions(); return [...p.home.map(h=>({...h,team:"home"})),...p.away.map(a=>({...a,team:"away"}))]; });
  ...
  const reset = () => { const p=initialPositions(); setPlayers([...p.home.map(h=>({...h,team:"home"})),...p.away.map(a=>({...a,team:"away"}))]); setBall({...initialBall}); setBallOffset({x:0,y:0}); setTrails({}); setAttachMode(false); setShowBallTrail(false); };
  ...
  const PitchMarkings = () => {
    ...
    const arc2=(cx_m,cy_m,r_m,dir,clipMin,clipMax)=>{
      const pts=[]; const n=200;
      for(let i=0;i<=n;i++){const t=dir==='down'?Math.PI*i/n:Math.PI+Math.PI*i/n; const xm=cx_m+r_m*Math.cos(t),ym=cy_m+r_m*Math.sin(t); const xp=mx2(xm),yp=my2(ym); if(xp>=0&&xp<=pw&&yp>=clipMin&&yp<=clipMax) pts.push({x:xp,y:yp});}
      if(pts.length<2) return ""; return "M "+pts.map(p=>`${p.x} ${p.y}`).join(" L ");
    };
    return (
      <g transform={`translate(${lox},${loy})`}>
        ...
        <line x1={cx2-ghw} y1={-9} x2={cx2-ghw} y2={2} stroke="white" strokeWidth="3" />
        <line x1={cx2+ghw} y1={-9} x2={cx2+ghw} y2={2} stroke="white" strokeWidth="3" />
        <line x1={cx2-ghw} y1={-4} x2={cx2+ghw} y2={-4} stroke="white" strokeWidth="2" />
        ...
        <line x1={cx2-ghw} y1={ph-2} x2={cx2-ghw} y2={ph+9} stroke="white" strokeWidth="3" />
        <line x1={cx2+ghw} y1={ph-2} x2={cx2+ghw} y2={ph+9} stroke="white" strokeWidth="3" />
        <line x1={cx2-ghw} y1={ph+4} x2={cx2+ghw} y2={ph+4} stroke="white" strokeWidth="2" />
      </g>
    );
  };
  ...
  const BoardView = () => (
    <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", padding:"8px", overflow:"auto", minWidth:0 }}>
      ...
      <svg ref={svgRef} viewBox={`0 0 ${PITCH_W} ${PITCH_H}`} style={{ width:"100%", maxWidth:680, borderRadius:8, touchAction:"none", userSelect:"none" }} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerLeave={handlePointerUp}>
        ...
        <PitchMarkings />
        {showLanes && (()=>{
          const pw2=600,lox2=(PITCH_W-pw2)/2,loy2=(PITCH_H-580)/2,laneW=pw2/7;
          const colors=["rgba(255,100,100,0.12)","rgba(100,150,255,0.12)","rgba(255,200,50,0.12)","rgba(100,255,150,0.12)","rgba(255,200,50,0.12)","rgba(100,150,255,0.12)","rgba(255,100,100,0.12)"];
          return <g>{colors.map((c,i)=><rect key={i} x={lox2+i*laneW} y={loy2} width={laneW} height={580} fill={c} />)}{[1,2,3,4,5,6].map(i=><line key={`ld${i}`} x1={lox2+i*laneW} y1={loy2} x2={lox2+i*laneW} y2={loy2+580} stroke="rgba(255,255,255,0.25)" strokeWidth="1" strokeDasharray="6,4" />)}</g>;
        })()}
  ...
  return (
    <div style={{ display:"flex", height:"100vh", fontFamily:"system-ui,sans-serif", background:"#1a1a2e", overflow:"hidden" }}>
      <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", minWidth:0 }}>
        ...
        {activeTab==="principles" ? <PrinciplesTab /> : <BoardView />}
      </div>
